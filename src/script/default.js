...
// TEORICOS ANUALES
function datos_anuales_teoricos() {
...
function mes() {
	if (peticion_http.readyState == READY_STATE_COMPLETE) {
		if (peticion_http.status == 200) {
			var respuesta_json = peticion_http.responseText;
			DATOS.Entradas = eval("(" + respuesta_json + ")");
			// INICIALIZAR
			editor_marcajes();
			...
			cargaContenido('/editor?accion=6&grupo=' + actual.grupo + '&id=' + actual.idMes + '&v1=' + mensual.dias_lab + '&v2=' + mensual.horas_trab, 'GET');
			//
...
function deselec_nota(Obj, n) {
	Obj.style.whiteSpace = 'nowrap';
	if (n == -1 && Obj.innerHTML != '') {
		editaNota(Obj);
		return false;
	}
	if (DATOS.Entradas.Notas[n]) {
		if (Obj.innerHTML !== Base64.decode(DATOS.Entradas.Notas[n][1])) {
			editaNota(Obj);
		}
	}
}
function selec_nota(Obj) {
...
function configuracion() {
	if (peticion_http.readyState == READY_STATE_COMPLETE) {
		if (peticion_http.status == 200) {
			var respuesta_json = peticion_http.responseText;
			DATOS.Normas = eval("(" + respuesta_json + ")");
...
function editado() {
	...
	if (peticion_http.readyState == READY_STATE_COMPLETE) {
		if (peticion_http.status == 200) {
			if (peticion_http.responseTex) alert(peticion_http.responseTex);
			cargaGrupo(actual.grupo, 0, mes);

		}
	}
}
function editadoNorma() {
	if (peticion_http.readyState == READY_STATE_COMPLETE) {
		if (peticion_http.status == 200) {
			if (peticion_http.responseTex) alert(peticion_http.responseTex);
...
function editaMarca(Obj, tipo) {
	...
	cargaContenido('/editor?accion=1&grupo=' + actual.grupo + '&id=' + (id / 1000) + '&tabla=Entradas&valor=' + (valor / 1000) + '&tipo=' + tipo, 'GET', editado);
}
function editaNota(Obj) {
	...
	cargaContenido('/editor?accion=1&grupo=' + actual.grupo + '&id=' + (id / 1000) + '&tabla=Notas&valor=' + valor, 'GET', editado);
}
function editaDia(Obj) {
	...
	var id = new Date(actual.fecha[0], actual.fecha[1], (actual.id), 0, 0, 0, 0).getTime();
	cargaContenido('/editor?accion=1&grupo=' + actual.grupo + '&id=' + (id / 1000) + '&tabla=Dias&valor=' + valor, 'GET', editado);
}
function edita_norma(Obj, min, max) {
	...
	cargaContenido('/editor?accion=5&grupo=' + actual.grupo + '&id=' + id + '&valor=' + valor, 'GET', editadoNorma);
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function cargaGrupo(n, accion, funcion) {
	var rango = '';
	if (accion == 0) {
		rango = '&' + rango_mes();
	} else if (accion == 3) {
...
var comprovar = ob => {
	...
	cargaContenido('/autentificacion?usuario=' + usuario + '&contrasena=' + contrasena, 'GET', () => {
		if (peticion_http.readyState == READY_STATE_COMPLETE) {
			if (peticion_http.status == 200) {
...
function elimina(Obj) {
	if (Obj.parentNode.id != 0) {
		var id = Number(Obj.parentNode.id) / 1000;
		cargando(1);
		cargaContenido('/editor?accion=2&grupo=' + actual.grupo + '&id=' + id, 'GET', editado);
	}
}
function inicia() {
	document.getElementById('control').innerHTML = '<div id="navegador"><span class="activo">Inicio</span></div>';
	cargando(1);
	cargaContenido('/editor', 'GET', menu);
}
function cargando(n) {
